...
class SyncController extends BaseController
{
    public function refresh(Request $request, Response $response, $args)
    {
        ...
        if ($this->isAuthorized($model)) {
            ...
            $results = $helper->getWithInFromDatabase(new PersonMetaData(), "guid", $guids);
    ...
    public function update(Request $request, Response $response, $args)
    {
        ...
        if ($this->isAuthorized($model)) {
            ...
            //update database
            $contentId = null;
            $exiting = $helper->getSingleFromDatabase(new PersonMetaData(), "guid=:guid", array("guid" => $model->ServerId));
            if ($exiting != null) {
    ...
    public function readCollectionEntries(Request $request, Response $response, $args)
    {
        ...
        if ($this->isAuthorized($model)) {
            ...
            if (count($model->KnownServerIds) > 0) {
                ...
                $contents = $helper->getWithInFromDatabase(new PersonMetaData(), "guid", $guids, true, "relation_id=:relation_id", array("relation_id" => $model->RelationId));
            } else {
                $contents = $helper->getFromDatabase(new PersonMetaData(), "relation_id=:relation_id", array("relation_id" => $model->RelationId));
            }
    ...
    public function getHistory(Request $request, Response $response, $args)
    {
        ...
        if ($this->isAuthorized($model)) {
            ...
            $helper = $this->getDatabaseHelper();
            $collection = $helper->getSingleFromDatabase(new PersonMetaData(), "guid=:guid", array("guid" => $model->ServerId));
            if ($collection != null) {
                //get all entries
                $historyEntries = $helper->getFromDatabase(new ProfessionInfo(), "content_id=:content_id", array("content_id" => $collection->id), "creation_date_time DESC");
                ...
                $helper = $this->getDatabaseHelper();
                $devices = $helper->getWithInFromDatabase(new Professions(), "id", $ids);
