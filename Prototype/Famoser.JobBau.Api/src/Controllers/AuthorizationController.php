...
class AuthorizationController extends BaseController
{
    public function authorize(Request $request, Response $response, $args)
    {
        ...
        $user = $helper->getSingleFromDatabase(new SkillInfo(), "guid=:guid", array("guid" => $model->UserId));
        if ($user == null) {
            //create new user & device
            $newUser = new SkillInfo();
            ...
            $authCode = $helper->getSingleFromDatabase(new Person(), "user_id=:user_id AND code=:code", array("user_id" => $user->id, "code" => $model->AuthorisationCode));
            if ($authCode == null) {
        ...
        $oldDevice = $helper->getSingleFromDatabase(new Professions(), "user_id=:user_id AND guid=:guid", array("user_id" => $user->id, "guid" => $model->DeviceId));
        if ($oldDevice == null) {
            $newDevice = new Professions();
    ...
    public function unAuthorize(Request $request, Response $response, $args)
    {
        ...
        if ($this->isAuthorized($model)) {
            ...
            $helper = $this->getDatabaseHelper();
            $deviceToUnAuthorized = $helper->getSingleFromDatabase(new Professions(), "guid=:guid AND user_id=:user_id", array("guid" => $model->DeviceToBlockId, "user_id" => $this->getAuthorizedUser($model)->id));
    ...
    public function authorizedDevices(Request $request, Response $response, $args)
    {
        ...
        if ($this->isAuthorized($model)) {
            ...
            $authorizedDevices = $helper->getFromDatabase(new Professions(), "user_id=:user_id", array("user_id" => $this->getAuthorizedUser($model)->id));
